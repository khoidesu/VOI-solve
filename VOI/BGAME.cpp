...
void Solve()
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        {
            pll tmp = get(e.u, e.v);
            maximize(ans, min(tmp.fi, e.w) + max(tmp.se, e.w));
        }
    }
    cout << ans;
}
